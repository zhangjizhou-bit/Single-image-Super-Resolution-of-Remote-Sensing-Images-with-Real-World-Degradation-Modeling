...
def add_color_error(img, error_level1=0):
...
def add_Gaussian_noise(img, noise_level1=0, noise_level2=15):
...
def degradation_remote(img, sf=4, lq_patchsize=72, clean_up=True, clean_sf=2):
    """
    This is the degradation model of remote sensing image super-resolution
    ----------
    img: HXWXC, [0, 1], its size should be large than (lq_patchsizexsf)x(lq_patchsizexsf)
    sf: scale factor
    ...
    if clean_up:
        img = cv2.resize(img, (int(1/clean_sf*img.shape[1]), int(1/clean_sf*img.shape[0])), interpolation=cv2.INTER_AREA)
    hq = img.copy()

    # blur
    img = add_blur(img, sf=sf)
    # downsampling (binning)
    img = cv2.resize(img, (int(1/sf*img.shape[1]), int(1/sf*img.shape[0])), interpolation=cv2.INTER_AREA)
    # add noise
    img = add_Gaussian_noise(img, noise_level1=0, noise_level2=15)
    # add final JPEG compression noise
    img = add_JPEG_noise(img)
    # # random crop
    # img, hq = random_crop(img, hq, sf, lq_patchsize)
