...
def main():
    ...
    parser.add_argument('-opt', type=str, default = 'options/aid/train_kernel_noise.yml', help='Path to option YMAL file.')
    # parser.add_argument('-opt', type=str, default = 'options/aid/train_bicubic.yml', help='Path to option YMAL file.')
    ...
    if rank <= 0:  # normal training (rank -1) OR distributed training (rank 0)
        ...
        if opt['use_tb_logger'] and 'debug' not in opt['name']:
            ...
            tb_logger = SummaryWriter(log_dir='tb_logger/' + opt['name'] + '_' + time.strftime("%Y%m%d%H%M%S", time.localtime()))
    else:
